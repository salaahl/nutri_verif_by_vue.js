...
<template>
  <section id="search-bar" class="relative mb-16">
    <form @submit="onSubmit" id="search-form" class="relative">
      <div class="w-full flex items-center justify-between">
        ...
        <div class="relative w-full">
          ...
          <input type="text" id="search-input"
            ...
            :value="input" placeholder="Entrez un nom de produit, un code-barres, une marque ou un type d'aliment"
            required />
        </div>
        <button type="submit"
      ...
      <div class="radio-toolbar w-full flex flex-wrap items-center mt-4 md:mt-6 text-sm text-gray-700">
        <input type="radio" name="sort_by" id="popularity_key" value="popularity_key"
          :checked="filter === 'popularity_key' || !filter" />
        <label @click="onFilterChange"
          class="radio_label mt-2 md:mt-0 mr-4 text-sm font-semibold bg-gray-400 rounded-full"
          for="popularity_key">Popularité</label>

        <input type="radio" name="sort_by" id="product_name" value="product_name"
          :checked="filter === 'product_name'" />
        <label @click="onFilterChange"
          class="radio_label mt-2 md:mt-0 mr-4 text-sm font-semibold bg-gray-400 rounded-full"
          for="product_name">Pertinence</label>

        <input type="radio" name="sort_by" id="created_t" value="created_t" :checked="filter === 'created_t'" />
        <label @click="onFilterChange"
          class="radio_label mt-2 md:mt-0 mr-4 text-sm font-semibold bg-gray-400 rounded-full" for="created_t">Date
          d'ajout</label>
        ...
        <label @click="onFilterChange"
          class="radio_label mt-2 md:mt-0 mr-4 text-sm font-semibold bg-gray-400 rounded-full"
          for="nutriscore_score">Nutriscore</label>

        <input type="radio" name="sort_by" id="nova_score" value="nova_score" :checked="filter === 'nova_score'" />
        <label @click="onFilterChange"
          class="radio_label mt-2 md:mt-0 mr-4 text-sm font-semibold bg-gray-400 rounded-full" for="nova_score">Groupe
          Nova</label>
      </div>
    </form>
  </section>
