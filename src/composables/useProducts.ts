...
export function useProducts() {
  ...
  async function fetchLastProducts() {
    ...
    const route = isLocalhost
      ? '/data/mock-products.json'
      : `${API_BASE_URL}?&fields=${encodeURIComponent(fields)}&purchase_places_tags=france&sort_by=created_t&page_size=199&action=process&json=1`
    error.value = null
  ...
  async function fetchSuggestedProducts(
    ...
    let route = isLocalhost
      ? '/data/mock-products.json'
      : `${API_BASE_URL}?search_terms=${encodeURIComponent(name ?? brand.split(',')[0])}&categories_tags=${encodeURIComponent(categories.join('|'))}&fields=${encodeURIComponent(fields)}&purchase_places_tags=france&sort_by=nutriscore_score,nova_group,popularity_key&page_size=199&action=process&json=1`
